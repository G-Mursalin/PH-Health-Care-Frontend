...
const ProfileUpdateModal = ({ open, setOpen, id }: TProps) => {
  ...
  const submitHandler = async (values: FieldValues) => {
    // Create all specialitiesId array
    const filteredSpecialtiesIds = doctorData?.doctorSpecialties.map(
      (item: any) => item.specialitiesId
    );

    // Select only those ids that are not equal to saved doctor specialitiesId
    const result = selectedSpecialtiesIds.filter(
      (id) => !filteredSpecialtiesIds.includes(id)
    );

    const specialties = result.map((val: string) => {
      return {
        specialtiesId: val,
        isDeleted: false,
      };
